...
26. **Caso: agregar `@ratelimit` a `login_view` (un fix de seguridad legítimo) rompió el login en producción**, porque el `CACHES` de producción apuntaba por defecto a un Redis en `127.0.0.1` que nunca existió — ya se sabía que era un riesgo latente (afectaba otras 3 rutas con `@ratelimit`), pero nadie lo había disparado porque esas rutas casi no se usan. *¿Había una regla?* No, faltaba una: **antes de agregar `@ratelimit`/`@cache_page`/cualquier cosa que dependa de `CACHES` a una ruta de alto tráfico, confirmar en el entorno real (no solo en local) que el backend de caché configurado responde.** Se escribió en [[data-rules]] y en el default de `settings.py` (nunca más un default de Redis en localhost — cae a `LocMemCache` si no hay `REDIS_URL` real).
27. **Caso: `tech_debt_fixes.py` se escribió para centralizar cálculos duplicados, pero nunca se conectó al código real — solo lo prueban sus propios tests.** *¿Había una regla?* No había una que exigiera verificar que una "solución a deuda técnica" esté realmente en el camino de ejecución, no solo probada de forma aislada. Nueva regla, en [[evolution-rules]] mismo: **cuando se declare resuelta una deuda técnica, se confirma con grep que el código viejo ya no existe o ya llama al nuevo — un módulo de reemplazo sin nadie importándolo no resolvió nada.**
28. La regla nueva lleva su caso pegado (ver arriba). Si en el futuro aparece otro caso parecido, se afila aquí mismo.
