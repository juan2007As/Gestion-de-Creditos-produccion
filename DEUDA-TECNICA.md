...
| 11 | ~~El CI no era un gate real~~ **[RESUELTO]** `bandit`/`safety` ya no corren con `\|\| true`. Nuevo job `secrets-scan`. `quality-gate` falla de verdad si `security` o `secrets-scan` fallan. **`render.yaml` ya tiene `autoDeploy: false`**, y se agregó el job `deploy` en `.github/workflows/tests.yml` que dispara el Deploy Hook de Render (`secrets.RENDER_DEPLOY_HOOK_URL`, ya generado por el dueño) solo si `quality-gate` pasó, solo en push a `main`. | `.github/workflows/tests.yml`, `render.yaml:22` | El gate de calidad ya bloquea de verdad y ahora también controla el deploy real. | release-rules §1 / dependencies-rules |
